...
class Hexagon {
	...
	public var corners(get, never):Array<Vector2D>;
	...
	public function get_corners():Array<Vector2D> {
		var H_OFFSET = SQRT3 * 0.5 * radius;
		var V_OFFSET = radius * 0.5;
		return [
			new Vector2D(centerX - H_OFFSET, centerY - V_OFFSET),
			new Vector2D(centerX, centerY - radius),
			new Vector2D(centerX + H_OFFSET, centerY - V_OFFSET),
			new Vector2D(centerX + H_OFFSET, centerY + V_OFFSET),
			new Vector2D(centerX, centerY + radius),
			new Vector2D(centerX - H_OFFSET, centerY + V_OFFSET)
